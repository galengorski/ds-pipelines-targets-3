...
tar_option_set(packages = c("tidyverse", "dataRetrieval", "urbnmapr", "rnaturalearth", "cowplot", "leaflet","leafpop","htmlwidgets"))
...
list(
  ...
  mapped_by_state_targets <- tar_map(
    ...
    tar_target(nwis_data, retry::retry(get_site_data(nwis_inventory, state_abb, parameter), when = 'Ugh, the internet data transfer failed! Try again.', max_tries = 10)),
  ...
  tar_target(timeseries_map_html,
             map_timeseries(oldest_active_sites,
                            summary_state_timeseries_csv,
